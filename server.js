...
/* 1. INITIALIZE CLIENTS (Add this above your routes) */
...
app.post('/send-code', async (req, res) => {
    ...
    try {
        ...
        await transporter.sendMail({
            from: `"TAM Evolution" <${process.env.EMAIL_USER}>`, // Fixed string
            to: email,
        ...
        res.status(500).json({ success: false, message: "Email/Redis Error" });
    }
});

// PHASE 2: Verify and Signup
app.post('/signup', async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query(query, [username, email, hashedPassword, dob]);
...
).json({ success: false, message: "Redis/Mail Error" });
    }
});
